...
public class TableFormatDiscoverer {
	...
	public BasicType[] discoverTypesFor(PrimitiveColumn column){
		//for(Cell cellCopy : column.getValues()) if(cellCopy instanceof TableCell) return null;
		...
		BasicType[] basicTypes = new BasicType[5];
		basicTypes[0] = discoverTypesFor(column.getValues(), VALIDATE_RATE_75);
		basicTypes[1] = discoverTypesFor(column.getValues(), VALIDATE_RATE_70);
		basicTypes[2] = discoverTypesFor(column.getValues(), VALIDATE_RATE_65);
		basicTypes[3] = discoverTypesFor(column.getValues(), VALIDATE_RATE_60);
		basicTypes[4] = discoverTypesFor(column.getValues(), VALIDATE_RATE_55);
   ...
   /*public Enumeration discoverEnumerationFor(List<Cell> list){
	...
	public Enumeration discoverEnumerationFor(PrimitiveColumn column){
		   List<String> copy = new ArrayList<String>(column.getValues());
		   ...
		   if(copy.size() < (column.getValues().size()*(1-ENUM_DISP))){
			   Enumeration enumeration = DataDslFactory.eINSTANCE.createEnumeration();
			   enumeration.getValues().addAll(copy);
			   return enumeration;
		   }
   ...
   /*public Row getRandomRow(Table table){
		Random rnd = new Random();
		int rowPosition = (int)(rnd.nextDouble() * table.getRows().size());
		return table.getRows().get(rowPosition);
	}*/
	
	/*public List<Cell> getColumnContentData(Table table, int i){
	...
	/*public int getWidthTable(Table table){
	...
	public List<Enumeration> getEnumerations(Table table){        
       try {
           List<Enumeration> enumerations = new ArrayList<Enumeration>();
           TableFormatDiscoverer discoverer = new TableFormatDiscoverer();
           
           //int max_size_row = getWidthTable(table);
           for(Column column : table.getColumns()){
        	   if(column instanceof PrimitiveColumn){
        		   enumerations.add(discoverer.discoverEnumerationFor((PrimitiveColumn) column));
        	   }
           }
           
           /*for(int i = 0; i<max_size_row; i++){
           	enumerations.add(discoverer.discoverEnumerationFor(getColumnContentData(table, i)));
           }*/
	...
	public List<BasicType[]> getTypes(Table table){        
       try {
           ...
           for(Column column : table.getColumns()){
        	   if(column instanceof PrimitiveColumn){
        		   basictypes.add(discoverer.discoverTypesFor((PrimitiveColumn) column));
        	   }
        	   //basictypes.add(discoverer.discoverTypesFor(getColumnContentData(table, i)));
